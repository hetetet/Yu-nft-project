...
const InfoCard = ({ value }: { value?: any }) => {
  const { asset_contract, token_id } = value
  const { chain, address } = asset_contract

  const url =
    chain === 'ETHEREUM' || chain === undefined
      ? `/assets/${address}/${token_id}`
      : `/assets/${chain}/${address}/${token_id}`
    ...
    <Link href={url} passHref>
      <div className={style.wrapper}>
        ...
        <div className={style.bottomWrap}>
          ...
          <div>
            <img className={style.icon} src={value.image_url} />
          </div>
        </div>
      </div>
    </Link>
